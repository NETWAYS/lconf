...
# default test infos
$testinfo->{description}->{short} = 'test a service and inheritance of a single attribute';
...
sub test {
	...
	$result = testAddLdif($ldap, 'cases/1011_service_inheritance_single_attribute.ldif');
	if ($result->{code} != 0) { $testresult->{code} = $result->{code}; $testresult->{message} = $result->{message}; return $testresult; }
