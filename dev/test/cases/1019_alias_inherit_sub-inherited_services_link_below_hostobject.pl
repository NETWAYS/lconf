...
# default test infos
$testinfo->{description}->{short} = 'test alias with sub-inherited services; link below hostobject';
...
sub test {
	...
	$result = testAddLdif($ldap, 'cases/1019_alias_inherit_sub-inherited_services_link_below_hostobject.ldif');
	if ($result->{code} != 0) { $testresult->{code} = $result->{code}; $testresult->{message} = $result->{message}; return $testresult; }
	...
	my $success_count = 0;
		...
		# --> service sub-inherited from alias; alias below hostobject
		if (!defined $data->{'SERVICES'}->{'example-service-4'}) {
			$testresult->{message} = "service 'example-service-4' not found";
			$testresult->{code} = 2;
		} else { $success_count++; }
