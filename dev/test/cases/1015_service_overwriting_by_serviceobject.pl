...
# default test infos
$testinfo->{description}->{short} = 'test service overwriting by service object';
...
sub test {
	...
	$result = testAddLdif($ldap, 'cases/1015_service_overwriting_by_serviceobject.ldif');
	if ($result->{code} != 0) { $testresult->{code} = $result->{code}; $testresult->{message} = $result->{message}; return $testresult; }
	...
	my $success_count = 0;
		...
		# service object was written and it's contact is 'another-testuser'?
		if ($data->{'SERVICES'}->{'example-service'}->{'contacts'} ne 'another-testuser') {
			$testresult->{message} = "service's contact != 'another-testuser'; value is set to '$data->{'SERVICES'}->{'example-service'}->{'contacts'}'";
			$testresult->{code} = 2;
		} else { $success_count++; }
