...
# default test infos
$testinfo->{description}->{short} = 'test alias with a single service; link below hostobject';
...
sub test {
	...
	$result = testAddLdif($ldap, 'cases/1018_alias_inherit_single_service_link_below_hostobject.ldif');
	if ($result->{code} != 0) { $testresult->{code} = $result->{code}; $testresult->{message} = $result->{message}; return $testresult; }
