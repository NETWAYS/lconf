...
# default test infos
$testinfo->{description}->{short} = 'test service overwriting due to the tree';
...
sub test {
	...
	$result = testAddLdif($ldap, 'cases/1013_service_overwriting_due_to_the_tree.ldif');
	if ($result->{code} != 0) { $testresult->{code} = $result->{code}; $testresult->{message} = $result->{message}; return $testresult; }
	...
	my $success_count = 0;
		...
		# service object was written and it's contact is 'testuser'?
		if ($data->{'SERVICES'}->{'example-service'}->{'contacts'} ne 'testuser') {
			$testresult->{message} = "service's contact != 'testuser'; value is set to '$data->{'SERVICES'}->{'example-service'}->{'contacts'}'";
			$testresult->{code} = 2;
		} else { $success_count++; }
