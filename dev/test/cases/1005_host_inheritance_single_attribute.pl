...
# default test infos
$testinfo->{description}->{short} = 'test host inheritance with a single attribute';
...
sub test {
	...
	$result = testAddLdif($ldap, 'cases/1005_host_inheritance_single_attribute.ldif');
	if ($result->{code} != 0) { $testresult->{code} = $result->{code}; $testresult->{message} = $result->{message}; return $testresult; }
	...
	my $success_count = 0;
		...
		# host's contact is 'icingaadmin'?
		if ($data->{'HOSTS'}->{'example-host'}->{'contacts'} ne 'icingaadmin') {
			$testresult->{message} = "host's contact != 'icingaadmin'; value is set to '$data->{'HOSTS'}->{'example-host'}->{'contacts'}'";
			$testresult->{code} = 2;
		} else { $success_count++; }
