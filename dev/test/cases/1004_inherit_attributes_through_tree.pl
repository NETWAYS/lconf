...
#
# inherit a host and an service attribute through the main tree
#
# tredel, 2011
#
...
$testresult->{description} = 'inheritance of a host and an service attribute through the main tree?';
...
sub test {
	...
	my $result = initializeLDAP('1004_inherit_attributes_through_tree.ldif');
	...
	if (defined($data->{SERVICES}->{testservice}->{testhost}->{contacts}) && $data->{SERVICES}->{testservice}->{testhost}->{contacts} ne 'test-contact-service') {
		$testresult->{message} = "contacs on service 'testservice' != 'test-contact-service'; contacts = $data->{SERVICES}->{testservice}->{testhost}->{contacts}";
		$testresult->{code} = 2;
	} else { $counter++; }
